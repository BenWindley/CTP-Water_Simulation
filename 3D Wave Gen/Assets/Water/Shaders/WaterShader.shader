Shader "Unlit/WaterShader"
{
	Properties
	{
		...
		_LightColour("Light Coliur", Color) = (0, 0, 0)
		...
		_MeshBounds("Mesh Bounds", Float) = 326.4
	}
	...
	SubShader
	{
		...
		Pass
		{
			...
			{
				return _Amplitude * float4(tex2Dlod(_DisplacementMap, float4(world_position.xz / (_MeshBounds / _MainTex_ST.x), 0, 0)).xyz, 1);
			}

			float3 GetNormalAtWorldPosition(float3 world_position)
			{
				return _Amplitude * float4(tex2Dlod(_NormalMap, float4(world_position.xz / (_MeshBounds / _MainTex_ST.x), 0, 0)).xyz, 1);
			}
			...
			void geom(triangle v2g input[3], inout TriangleStream<g2f> TriStream)
			{
				...
				for(int i = 0; i < 3; ++i)
				{
					...
					displacement = _Amplitude * float4(GetDisplacementAtWorldPosition(mul(unity_ObjectToWorld, input[i].vertex)).xyz, 1);
					output.normal = GetNormalAtWorldPosition(mul(unity_ObjectToWorld, input[i].vertex)).xyz;
			...
			fixed4 frag(g2f i) : SV_Target
			{
				...
				return _LightColour * specular_colour + 0.8f * col + _RefractionIntensity * lerp(refraction_col, reflection_col, fresnel);
			}
			ENDCG
		}
	}
